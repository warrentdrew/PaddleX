...
class BEVDet3DPredictor(BasicPredictor):
    ...
    def _format_output(
        ...
        results["input_path"] = input_ids
        results["input_img"] = input_imgs
        results["input_points"] = input_points
        results["input_lidar2img"] = input_lidar2imgs
    ...
    def process(self, batch_data: List[str]) -> Dict[str, Any]:
        ...
        infer_input, sample_token = self.pre_tfs["GetInferInput"](sample=sample)
        infer_output = self.infer(x=infer_input)
        results = self._format_output(infer_input, infer_output, sample_token)
        return results
